...
**Phases 0–4 complete + post-MVP polish & cost tracking**, tagged through **`v0.4.1`**. **Phase 8 (Documents) complete** — tagged **`v0.5.0`** (the last legacy unprefixed iOS tag). **Phase 5 — Apple Sign-In & Beta is done now that the Apple Developer Program is approved**: native Sign in with Apple shipped, the app is on TestFlight (Build 1 approved for external testing), plus a freemium **paywall** (free up to 4 subscriptions), the **spend-hero** Home header, the **"subscriptions"** rename, and the **"how it's paid"** feature (billing source + optional payment method). Tagged **`ios-v0.6.0`** — the first platform-prefixed iOS tag — with **Build 2 (version 1.0)** archived from it. **`ios-v0.6.1`** (Build 3) follows with **optional URL** (a subscription no longer needs a website; no-website tiles show the full title in the circle). **`ios-v0.6.2`** (Build 5) adds **spending tools** — a Monthly/Yearly toggle on the total, **flexible billing cadences** (weekly … every 2 months … yearly), chip pickers, and a **cost breakdown screen** (roadmap #8) — plus the TestFlight mock-unlock. Phase 7 (Reminders) + smaller ideas (#8–11, incl. the cost breakdown screen) remain. RevenueCat wiring + server-side entitlement enforcement are deferred until store products are configured. The Android app mirrors features on its own **`android-v*`** cadence — now at **`android-v0.6.1`**, level with iOS through the v0.6.2 spending tools (Monthly/Yearly toggle, flexible cadences + chips, cost breakdown) on top of the v0.6 set, polish pass, and brand-blue theme; next is Play Store prep (`android-v0.7.0`). See `project-manager-log.md` / `docs/roadmap.md`.
